...
class Can_interface
{
    ...
    Device<AQ_acceleration> aq_acceleration{AQ_ACCELERATION_CAN_ID, 4};
    Device<AQ_gyroscope> aq_gyroscope{AQ_GYROSCOPE_CAN_ID, 8};
    Device<BMS_HV_main> bms_hv_main{BMS_HV_MAIN_CAN_ID, 16};
    Device<BMS_LV_main> bms_lv_main{BMS_LV_MAIN_CAN_ID, 32};
    Device<BMS_LV_temperature> bms_lv_temperature{BMS_LV_TEMPERATURE_CAN_ID,
                                                  64};
    Device<Dash_Main> dash_main{DASH_MAIN_CAN_ID, 128};
    Device<Dash_TCS> dash_TCS{DASH_TCS_CAN_ID, 1};
    Device<Dash_Smart_Fuses_FAN_speed> dash_fan_speed{
        DASH_SMART_FUSES_FAN_SPEED_CAN_ID, 2};
    Device<Dash_steering_wheel_request> dash_steering_request{
        DASH_STEERING_WHEEL_REQUEST_CAN_ID, 4};
    Device<Dash_lap_finished> dash_lap_finished{DASH_LAP_FINISHED_CAN_ID, 8};
    Device<Lap_timer_Main> laptimer_main{LAP_TIMER_MAIN_CAN_ID, 16};
    Device<Lap_timer_Pass> laptimer_pass{LAP_TIMER_PASS_CAN_ID, 32};
    Device<SF_main> sf_main{SF_MAIN_CAN_ID, 64};
    Device<SF_FrontBox> sf_frontbox{SF_FRONTBOX_CAN_ID, 128};
    Device<SF_CoolingAndVSafety> sf_coolingandvsafety{
        SF_COOLINGANDVSAFETY_CAN_ID, 1};
    Device<SF_DV> sf_dv{SF_DV_CAN_ID, 2};
    Device<SF_WS> sf_ws{SF_WS_CAN_ID, 4};
    Device<SF_NUCS> sf_nucs{SF_NUCS_CAN_ID, 8};
    ...
    Device<Steering_Wheel_event> steering_wheel_event{
        STEERING_WHEEL_EVENT_CAN_ID, 32};
    Device<TC_main> tc_main{TC_MAIN_CAN_ID, 64};
    ...
    Device<TC_wheel_velocities> tc_wheel_velocities{TC_WHEEL_VELOCITIES_CAN_ID,
                                                    1};
    Device<TC_temperatures> tc_temperatures{TC_TEMPERATURES_CAN_ID, 2};
    Device<TC_imu_acc> tc_imu_acc{TC_IMU_ACC_CAN_ID, 4};
    Device<TC_imu_gyro> tc_imu_gyro{TC_IMU_GYRO_CAN_ID, 8};
    Device<Telemetry_Main> telemetry_main{TELEMETRY_MAIN_CAN_ID, 16};
    Device<WheelTemp_main> wheel_temp_main{WHEELTEMP_MAIN_CAN_ID, 32};
    Device<YawProbe_air_flow> yawprobe_air_flow{YAWPROBE_AIR_FLOW_CAN_ID, 64};
...
public:
    ...
    bool get_aq_acceleration_new_data()
    {
        return aq_acceleration.get_new_data();
    }
    ...
    bool get_sf_cooling_new_data()
    {
        return sf_coolingandvsafety.get_new_data();
    }
    ...
    bool get_tc_temperatures_new_data()
    {
        return tc_temperatures.get_new_data();
    }
